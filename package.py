...
def build_docx(path):
    ...
    feature_table(doc, [
        ...
        ("🔒 桌面锁定", "一键扫描桌面图标、逐图标锁定（持久化）；锁定后图标无法选中与拖动，双击仍可打开软件"),
        ("📹 录屏", "全局快捷键（默认 Ctrl+H）一键开始/停止录制；可截选录制区域（默认全屏）；画面+音频存为 MP4"),
    ...
    h(doc, "十、录屏（v4.0）")
    para(doc, "左侧导航进入「📹 录屏」：")
    bullet(doc, "全局快捷键：默认 Ctrl+H（可自定义 Ctrl/Alt/Shift + 字母），任意界面按下即开始/停止录制；")
    bullet(doc, "录制区域：点击「✂ 截选录制区域」在全屏半透明选择框上拖拽框选，默认全屏；")
    bullet(doc, "录制内容：桌面画面（gdigrab）+ 音频（系统声音/麦克风，dshow），输出 MP4；")
    bullet(doc, "保存位置：可自定义，默认 用户\\视频 目录；录制中实时显示时长，停止后可打开所在文件夹。")
    para(doc, "说明：录屏基于系统 ffmpeg（gdigrab/dshow），需安装 ffmpeg 并加入 PATH（https://ffmpeg.org）。")

    h(doc, "十一、工具与宠物")
    para(doc, "工具：内置 Windows 7 经典小工具库（时钟/日历/CPU 仪表盘/天气/货币/资讯/幻灯片/拼图），"
              "点击「📥 安装」联网下载并解压到本地（%LOCALAPPDATA%\\WindowsPP\\gadgets），「▶ 打开」以独立程序窗口运行，"
              "支持卸载与开机自启动。提示：Windows 10/11 已移除侧边栏平台，小工具以兼容方式显示。")
